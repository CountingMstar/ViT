...
class SAC_countinuous:
    ...
    def train(
        ...
        with torch.no_grad():
            # print("7777777777")
            # print(s_next.shape)
            # s_next = s_next.transpose(0, 1)
            # print(s_next.shape)
            # s_next = s_next.contiguous().view(1, -1)
            # print(s_next.shape)
            # print(s_next.shape[0])
            # s_next = s_next.view(-1, s_next.shape[0] * s_next.shape[1])
    ...
    def save(self, EnvName, timestep):
        torch.save(
            self.actor.state_dict(), "./model/{}_actor{}.pth".format(EnvName, timestep)
        )
        torch.save(
    ...
    def load(self, EnvName, timestep):
        self.actor.load_state_dict(
            torch.load("./model/{}__actor{}.pth".format(EnvName, timestep))
        )
        self.q_critic.load_state_dict(
            torch.load("./model/{}__q_critic{}.pth".format(EnvName, timestep))
        )
...
class ReplayBuffer:
    ...
    def add(self, s, a, r, s_next, dw):
        ...
        # self.s[self.ptr] = torch.from_numpy(s.view(1, -1)[0].numpy()).to(self.dvc)
        # self.a[self.ptr] = torch.from_numpy(a.view(1, -1)[0].numpy()).to(
        #     self.dvc
        # )  # Note that a is numpy.array
        # self.r[self.ptr] = r
        # self.s_next[self.ptr] = torch.from_numpy(s_next.view(1, -1)[0].numpy()).to(
        #     self.dvc
        # )
        # self.dw[self.ptr] = dw

        self.s[self.ptr] = s.view(1, -1)[0].to(self.dvc)
        # print(self.a[self.ptr])
        # print(a)
        self.a[self.ptr] = a.view(1, -1)[0].to(self.dvc)  # Note that a is numpy.array
        self.r[self.ptr] = r
        self.s_next[self.ptr] = s_next.view(1, -1)[0].to(self.dvc)
        self.dw[self.ptr] = dw
