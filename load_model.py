...
parser.add_argument(
    "--dvc", type=str, default="cuda", help="running device: cuda or cpu"
)
parser.add_argument(
    "--EnvIdex", type=int, default=0, help="PV1, Lch_Cv2, Humanv4, HCv4, BWv3, BWHv3"
)
parser.add_argument(
    "--write",
    type=str2bool,
...
parser.add_argument(
    "--Loadmodel", type=str2bool, default=True, help="Load pretrained model or Not"
)
parser.add_argument("--ModelIdex", type=int, default=100, help="which model to load")

parser.add_argument("--seed", type=int, default=0, help="random seed")
parser.add_argument(
    "--Max_train_steps", type=int, default=int(100000), help="Max training steps"
)
parser.add_argument(
    "--save_interval",
    type=int,
...
parser.add_argument(
    "--net_width", type=int, default=256, help="Hidden net width, s_dim-400-300-a_dim"
)
...
parser.add_argument(
    "--adaptive_alpha", type=str2bool, default=True, help="Use adaptive_alpha or Not"
)
...
def main():
    ...
    ####################################
    d_model, max_len = 13, 5
    opt.state_dim = d_model * max_len
    opt.action_dim = d_model * max_len
    env = game.PE_GAME(d_model, max_len)
    eval_env = game.PE_GAME(d_model, max_len)

    train_start_ep = 10000
    ####################################
    # print("3333333333")
    # print(opt.state_dim)
    ...
    print(
        ...
        f"max_a:{opt.max_action}  min_a:{-opt.max_action}  max_e_steps:{opt.max_e_steps}"
    )
    ...
    #         """Interact & trian"""
    #         if total_steps < train_start_ep:
    #             act = torch.randn(d_model, max_len)
    ...
    #         act = act.view(d_model, max_len)
    #         s_next, r, done = env.step(act)  # dw: dead&win; tr: truncated
    ...
    #             print("#############################")
    #             print("Episodes: " + str(total_steps))
    #             print("Reward: " + str(ep_r))
    ...
    #         if total_steps % 10000 == 0 and total_steps >= train_start_ep:
    #             print("############### save model ##############")
    #             agent.save("PE_", int(total_steps / 100))

    #         # """train if it's time"""
    #         # # train 50 times every 50 steps rather than 1 training per step. Better!
    #         # if (total_steps >= 2 * opt.max_e_steps) and (
    #         #     total_steps % opt.update_every == 0
    #         # ):
